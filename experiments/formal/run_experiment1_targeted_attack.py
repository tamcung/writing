...
def attack_sqli_rows(
    ...
    for i, row in enumerate(sqli_rows):
        text = str(row["text"])
        result = targeted_evasion_search(
            source_text=text,
            score_fn=lambda batch: model.predict_proba(batch),
            seed=seed * 1_000_003 + i * 9_176 + len(text),
            operators=operators,
            steps=steps,
        ...
        if (i + 1) % 50 == 0 or i + 1 == len(sqli_rows):
            ...
            print(f"    attacked {i + 1}/{len(sqli_rows)} recent_success={success:.3f} recent_drop={drop:.4f}")
